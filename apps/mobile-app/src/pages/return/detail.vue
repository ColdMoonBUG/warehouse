...
import { buildReturnReceipt, printText } from '@/utils/bluetooth-printer'
...
async function printReturn() {
  if (!doc.value) return
  try {
    const receipt = buildReturnReceipt(doc.value, storeName.value, salespersonName.value, products.value)
