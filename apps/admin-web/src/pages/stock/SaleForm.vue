      ...
      <template #header>
        <div class="header-row">
          <div class="header-left">
            ...
            <span>销售单 {{ doc.code || '新建' }}</span>
            <el-tag :type="statusType(doc)">{{ statusLabel(doc) }}</el-tag>
            <el-tag :type="doc.paymentType === 'cash' ? 'success' : 'warning'">{{ paymentTypeLabel(doc.paymentType) }}</el-tag>
          </div>
          <div class="header-actions">
            ...
            <el-button type="success" @click="doSettle" :disabled="!canSettle">确认收款</el-button>
            <el-button @click="doUnsettle" :disabled="!canUnsettle">取消收款</el-button>
            <el-button @click="goCreateReturn" :disabled="!canCreateReturn">新建关联退货</el-button>
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="isMobile ? 24 : 8">
            <el-form-item label="付款方式">
              <el-radio-group v-model="doc.paymentType">
                <el-radio label="cash">现金</el-radio>
                <el-radio label="bill">单子</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
import { getSaleById, postSale, saveSale, settleSale, unsettleSale, voidSale } from '@/api/sale'
...
function applyDoc(detail: SaleDoc) {
  doc.value = {
    ...detail,
    paymentType: detail.paymentType || 'bill',
...
function statusLabel(detail: SaleDoc) {
  if (detail.status === 'draft') return '草稿'
  if (detail.status === 'voided') return '已作废'
  return detail.settled ? '已结清' : '未结清'
}

function statusType(detail: SaleDoc) {
  if (detail.status === 'draft') return 'info'
  if (detail.status === 'voided') return 'danger'
  return detail.settled ? 'success' : 'warning'
}
...
function onProductChange(row: SaleLine) {
  ...
  if (product) row.price = product.salePrice
  updateLine(row)
}

function amountSum() {
  return doc.value.lines.reduce((sum, line) => sum + line.qty * line.price, 0)
}
...
async function post() {
  ...
  ElMessage.success(doc.value.paymentType === 'cash' ? '过账并自动结清成功' : '过账成功')
}

async function doSettle() {
  if (!doc.value.id) return
  try {
    await ElMessageBox.confirm(`确认「${doc.value.code}」已收款？`, '确认收款')
    await settleSale(doc.value.id)
    const detail = await getSaleById(doc.value.id)
    if (detail) applyDoc(detail)
...
async function doUnsettle() {
  if (!doc.value.id) return
  try {
    await ElMessageBox.confirm(`确认取消「${doc.value.code}」的收款状态？`, '取消收款')
    await unsettleSale(doc.value.id)
    const detail = await getSaleById(doc.value.id)
    if (detail) applyDoc(detail)
...
function goCreateReturn() {
  if (!doc.value.id) return
  router.push(`/stock/return/new?saleId=${doc.value.id}`)
}
...
onMounted(async () => {
  ;[salespersonAccounts.value, stores.value, products.value, warehouses.value] = await Promise.all([
...
.mobile-line .line-row { display:flex; justify-content:space-between; align-items:center; gap:12px; margin-top:8px; }
.mobile-line .line-row:first-child { margin-top:0; }
.mobile-line .cols { align-items:stretch; }
.mobile-line .cols-3 .col { flex:1; }
.mobile-line .cols .col { flex:1; }
.mobile-line .label { font-size:12px; color:#94a3b8; margin-bottom:4px; }
.mobile-line .value { color:#e5e7eb; }
